...
export default function OwnerLayout() {
  ...
  const isTabBarHidden = hideTabBarForRoutes.some((r) => pathname.startsWith(r))
  ...
  return (
      ...
      screenOptions={({ route }: { route: { name: string } }) => ({
        ...
        tabBarIcon: ({ focused, color, size }: { focused: boolean; color: string; size: number }) => {
          ...
          switch (route.name) {
            ...
            case 'trips': // Hoạt động (Map với Trips)
              IconComponent = MaterialCommunityIcons
              // Icon 2 mũi tên ngược chiều giống hình
    ...
    >
      ...
      {/* 2. HOẠT ĐỘNG (Map vào trips) */}
      <Tabs.Screen name="trips" options={{ title: 'Hoạt động' }} />

      {/* 3. TÀI CHÍNH (Cần tạo file finance.tsx tương ứng) */}
      <Tabs.Screen name="finance" options={{ title: 'Tài chính' }} />

      {/* 4. TÀI KHOẢN */}
      <Tabs.Screen name="profile" options={{ title: 'Tài khoản' }} />
        {/* Note: only the main tabs are registered here so the tab bar shows
          exactly: home, trips, finance, profile. Other owner routes remain
          reachable via navigation (file-based routing), but are not part of
          the tab bar. */}

        {/* Hidden routes: create explicit screens with `href: null` so they are
          not displayed in the tab bar but remain navigable via file-based
          routing (deep links / navigation.navigate). */}
