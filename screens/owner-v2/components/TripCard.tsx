...
const getStatusStyle = (status: string) => {
  ...
  switch (s) {
    ...
    case 'AWAITING_PROVIDER_PAYMENT':
      return { text: 'Đang chờ', color: '#D97706', bg: '#FFFBEB', border: '#F59E0B' } // Cam
    case 'IN_PROGRESS':
    case 'VEHICLE_HANDOVER':
    case 'LOADING':
    case 'DELIVERED':
      return { text: 'Đang chạy', color: '#059669', bg: '#ECFDF5', border: '#10B981' } // Xanh lá
    case 'COMPLETED':
      return { text: 'Hoàn thành', color: '#1E3A8A', bg: '#EFF6FF', border: '#3B82F6' } // Xanh dương
    case 'CANCELLED':
      return { text: 'Đã hủy', color: '#DC2626', bg: '#FEF2F2', border: '#EF4444' } // Đỏ
    default:
    ...
    >
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.vehicleRow}>
             ...
             <Text style={styles.vehicleText}>
               {trip.vehiclePlate ? `${trip.vehiclePlate} • ${trip.vehicleType || ''}` : 'Chưa gán xe'}
             </Text>
          </View>
        </View>
      ...
      <View style={styles.routeContainer}>
        <View style={styles.routeRow}>
          ...
          <Text style={styles.routeText} numberOfLines={1}>{trip.startAddress || 'Điểm đi'}</Text>
        </View>
        ...
        <View style={styles.routeRow}>
          ...
          <Text style={styles.routeText} numberOfLines={1}>{trip.endAddress || 'Điểm đến'}</Text>
        </View>
      </View>

      {/* SUMMARY */}
      <View style={styles.summaryBox}>
        <Text style={styles.summaryText}>{trip.tripRouteSummary || 'Đang cập nhật lộ trình...'}</Text>
      </View>
