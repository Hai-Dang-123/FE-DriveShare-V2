...
const InputField = ({ label, value, onChange, placeholder, width = '100%', keyboardType = 'default', required = false }: any) => (
  <View style={{ width, marginBottom: 12 }}>
    {label && <Text style={styles.label}>{label} {required && <Text style={{color: COLORS.danger}}>*</Text>}</Text>}
    <TextInput
      style={styles.input}
      value={String(value || '')}
...
const DocumentUploadBox = ({ image, onPress, label }: { image: any, onPress: () => void, label: string }) => (
  <TouchableOpacity style={styles.docUploadBox} onPress={onPress}>
    {image ? (
      <Image source={{ uri: image.uri }} style={styles.docImage} resizeMode="cover" />
    ) : (
...
const VehicleFormModal: React.FC<Props> = ({ visible, onClose, onCreate }) => {
  ...
  useEffect(() => {
    if (!visible) {
      setForm({
        VehicleTypeId: '', PlateNumber: '', Model: '', Brand: '',
        YearOfManufacture: new Date().getFullYear(), Color: '',
        PayloadInKg: 0, VolumeInM3: 0, Features: [],
        VehicleImages: [], Documents: [],
      })
      setVehicleTypes([])
      setShowTypeList(false)
      return
    ...
    ;(async () => {
      try {
        ...
        const mapped: VehicleType[] = items.map((t: any) => ({
          vehicleTypeId: t.vehicleTypeId ?? t.VehicleTypeId ?? String(t.id ?? ''),
          vehicleTypeName: t.vehicleTypeName ?? t.VehicleTypeName ?? t.name ?? '',
  ...
  const pickImage = async (index?: number) => {
    ...
    const imageObj = { uri: result.assets[0].uri, base64: result.assets[0].base64 }
    setForm((p: any) => {
  ...
  const removeImage = (index: number) => {
    setForm((p: any) => ({ ...p, VehicleImages: p.VehicleImages.filter((_: any, i: number) => i !== index) }))
  }
  ...
  const pickDocumentFile = async (docIndex: number, which: 'FrontFile' | 'BackFile') => {
    ...
    const fileObj = { uri: result.assets[0].uri, base64: result.assets[0].base64 }
    setForm((p: any) => {
        ...
        <Pressable style={styles.modalContainer} onPress={(e) => e.stopPropagation()}>
          ...
          <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.row}>
              <View style={{ flex: 1, marginRight: 10 }}>
                ...
                >
                  <Text style={{ color: form.VehicleTypeId ? COLORS.text : '#9CA3AF' }}>
                    {vehicleTypes.find(vt => vt.vehicleTypeId === form.VehicleTypeId)?.vehicleTypeName || 'Chọn loại xe'}
                  </Text>
                ...
                <ScrollView nestedScrollEnabled style={{ maxHeight: 150 }}>
                  {vehicleTypes.map((vt) => (
                    <TouchableOpacity 
                      ...
                      onPress={() => { handleChange('VehicleTypeId', vt.vehicleTypeId); setShowTypeList(false) }}
                    >
                      <Text style={styles.dropdownItemText}>{vt.vehicleTypeName}</Text>
                    </TouchableOpacity>
                  ))}
                </ScrollView>
              </View>
            ...
            <View style={styles.row}>
              <InputField label="Model" width="32%" value={form.Model} onChange={(t: string) => handleChange('Model', t)} />
              <InputField label="Năm SX" width="32%" value={form.YearOfManufacture} onChange={(t: string) => handleChange('YearOfManufacture', Number(t) || 0)} keyboardType="numeric" />
              <InputField label="Màu sắc" width="32%" value={form.Color} onChange={(t: string) => handleChange('Color', t)} />
            </View>
            ...
            <View style={styles.row}>
              <InputField label="Tải trọng (kg)" width="48%" value={form.PayloadInKg} onChange={(t: string) => handleChange('PayloadInKg', Number(t) || 0)} keyboardType="numeric" />
              <InputField label="Thể tích (m³)" width="48%" value={form.VolumeInM3} onChange={(t: string) => handleChange('VolumeInM3', Number(t) || 0)} keyboardType="numeric" />
            </View>
            ...
            <View style={{ marginBottom: 12 }}>
              ...
              <TextInput
                ...
                multiline
                placeholder="Ví dụ: Bửng nâng, GPS (phân cách bằng dấu phẩy)"
                placeholderTextColor="#9CA3AF"
                value={Array.isArray(form.Features) ? form.Features.join(', ') : ''}
                onChangeText={(t) => handleChange('Features', t.split(',').map(s => s.trim()).filter(Boolean))}
              />
            </View>
            ...
            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.imageList}>
              ...
              {form.VehicleImages.map((img: any, index: number) => (
                <View key={index} style={styles.imageWrapper}>
                  <Image 
                    source={{ uri: img.uri ?? (img.base64 ? `data:image/jpeg;base64,${img.base64}` : img.vehicleImageURL) }} 
                    style={styles.thumbnail} 
                  />
                  <TouchableOpacity style={styles.removeBtn} onPress={() => removeImage(index)}>
                    <Ionicons name="close" size={10} color="#fff" />
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.replaceOverlay} onPress={() => pickImage(index)} />
                </View>
              ))}
            </ScrollView>
            ...
            <View style={[styles.row, { alignItems: 'center', marginBottom: 10 }]}>
              <Text style={[styles.sectionTitle, { marginBottom: 0 }]}>Tài liệu & Giấy tờ</Text>
              <TouchableOpacity onPress={addDocument} style={styles.addDocBtn}>
                <Ionicons name="add-circle-outline" size={18} color={COLORS.primary} />
                <Text style={{ color: COLORS.primary, fontWeight: '600', fontSize: 13 }}>Thêm hồ sơ</Text>
              </TouchableOpacity>
            </View>
              ...
              <View key={idx} style={styles.docCard}>
                <View style={styles.docHeader}>
                  ...
                  <TouchableOpacity onPress={() => removeDocument(idx)}>
                    <Text style={{ color: COLORS.danger, fontSize: 12, fontWeight: '600' }}>Xóa</Text>
                  </TouchableOpacity>
                </View>
                ...
                <DateInput
                  ...
                  onChange={(d: string | null) => {
                    ...
                    docs[idx] = { ...(docs[idx] || {}), ExpirationDate: d ?? '' }
          ...
          <View style={styles.footer}>
            ...
            <TouchableOpacity 
              ...
              disabled={submitting}
            >
              {submitting ? <ActivityIndicator color="#fff" /> : <Text style={styles.btnSubmitText}>Tạo Xe</Text>}
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  dropdownBtn: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
    borderWidth: 1, borderColor: COLORS.border, borderRadius: 8,
    paddingHorizontal: 12, paddingVertical: 10, backgroundColor: COLORS.inputBg
  },
  dropdownList: {
    borderWidth: 1, borderColor: COLORS.border, borderRadius: 8,
    marginTop: -8, marginBottom: 12, backgroundColor: '#fff', zIndex: 10
  },
  ...
  removeBtn: {
    ...
    backgroundColor: COLORS.textLight, width: 18, height: 18, borderRadius: 9,
  ...
  docUploadBox: {
    height: 80, borderWidth: 1, borderColor: '#D1D5DB', borderStyle: 'dashed',
    borderRadius: 8, justifyContent: 'center', alignItems: 'center', backgroundColor: '#fff'
  },
