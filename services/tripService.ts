...
const tripService = {
  ...
  async getByOwner(pageNumber = 1, pageSize = 20) {
    try {
      const res = await api.get(`api/trip/owner?pageNumber=${pageNumber}&pageSize=${pageSize}`)
      return res.data
    } catch (e: any) {
  ...
  async getTripsByDriver(pageNumber = 1, pageSize = 10) {
    try {
      const res = await api.get(`api/trip/driver?pageNumber=${pageNumber}&pageSize=${pageSize}`)
      return res.data
    } catch (e: any) {
  ...
  async getDriverContractPdfLink(contractId: string, includeTerms: boolean = true) {
    try {
      const res = await api.get(`api/TripDriverContract/pdf-link/${contractId}?includeTerms=${includeTerms}`)
      return res.data
    } catch (e: any) {
    ...
    async getTripsByProvider(pageNumber = 1, pageSize = 10) {
      try {
        const res = await api.get(`api/trip/provider?pageNumber=${pageNumber}&pageSize=${pageSize}`)
        return res.data
      } catch (e: any) {
      ...
      async signDeliveryRecordAsDriver(recordId: string) {
        try {
          ...
          const res = await api.post(`api/tripdeliveryrecord/sign-delivery-record?tripDeliveryRecordId=${recordId}`)
          return res.data
        } catch (e: any) {
      ...
      async sendSignOtp(recordId: string) {
        try {
          const res = await api.post(`api/tripdeliveryrecord/send-sign-otp/${recordId}`)
          return res.data
        } catch (e: any) {
...
,
      ...
      async getDeliveryRecordForContact(recordId: string, accessToken: string) {
        try {
          ...
          const res = await apiPublic.get(`api/tripdeliveryrecord/contact/view/${recordId}?accessToken=${encodeURIComponent(accessToken)}`)
          return res.data
        } catch (e: any) {
      ...
      async sendOtpToContact(recordId: string, accessToken: string) {
        try {
          ...
          const res = await apiPublic.post(`api/tripdeliveryrecord/contact/send-otp/${recordId}`, payload)
          return res.data
        } catch (e: any) {
      ...
      async signDeliveryRecordForContact(dto: { DeliveryRecordId: string; Otp: string; AccessToken: string }) {
        try {
          const res = await apiPublic.post(`api/tripdeliveryrecord/contact/sign`, dto)
          return res.data
        } catch (e: any) {
