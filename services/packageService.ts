...
const packageService = {
  async createPackage(payload: any) {
    try {
      ...
      appendIf(
        "OtherRequirements",
        getField("OtherRequirements", "otherRequirements") ?? ""
      );
      ...
      appendIf(
        "PostPackageId",
        getField("PostPackageId", "postPackageId") ?? ""
      );
      ...
      for (let i = 0; i < imagesRaw.length; i++) {
        ...
        if (typeof img === "string") uri = img;
          ...
          uri = img.uri ?? img.packageImageURL ?? img.url;
        if (!uri) continue;
        ...
        if (uri.startsWith("data:")) {
          try {
            ...
            form.append(
              "PackageImages",
              blob,
              `package-${Date.now()}-${i}.jpg`
            );
          } catch (e) {
            console.warn(
              "Failed to convert dataUrl to blob for package image",
              e
            );
          }
        } else if (
          ...
          // @ts-ignore
          form.append("PackageImages", {
            uri,
            name: `package-${Date.now()}-${i}.jpg`,
            type: "image/jpeg",
          });
      ...
      try {
        ...
        console.debug(
          "packageService.createPackage - appended PackageImages count:",
          appended
        );
      } catch (e) {
  ...
  async updatePackage(payload: any) {
    try {
      const dto = {
        ...
        HandlingAttributes:
          payload.handlingAttributes || payload.HandlingAttributes || [],
        OtherRequirements:
