...
export const useNotification = () => {
  ...
  const registerForPushNotificationsAsync = async () => {
    ...
    if (!isGranted) {
      // Request permissions
      const { ios: newIos, android: newAndroid } = await Notifications.requestPermissionsAsync()
      
      const newlyGranted = Platform.OS === 'ios'
  ...
  useEffect(() => {
    ...
    notificationListener.current = Notifications.addNotificationReceivedListener(notification => {
      console.log('📬 [Foreground] Nhận thông báo:', notification.request.content)
